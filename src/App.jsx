...
useEffect(()=>{document.body.style.background='#fdf8f0'},[])
const generate=async()=>{
if(!bullets.trim())return
setLoading(true);setEssay('')
try{
const res=await fetch('/api/generate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({bullets})})
const data=await res.json()
setEssay(data.essay||'No essay')
}catch{setEssay('API Error')}
setLoading(false)
}
return(
